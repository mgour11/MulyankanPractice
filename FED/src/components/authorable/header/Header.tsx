  ...
  return (
    <div className="bg-gray-150 ">
      <Grid variant="lg">
            ...
            {fields?.navLinks.map((navLink, index) => (
              <li
                ...
                className="py-2 border-t-white border-t-2 border-opacity-30 md:border-none">
					<Link key={index} href={navLink.url}  className="hover:no-underline text-black mr-4 font-sans">
        ...
        {/* <nav className="col-span-8 ml-auto ">
          {fields?.navLinks.map((navLink, index) => (
            <a className="mr-4 font-sans" key={index} href={navLink.url}>
              {navLink.fields.NavigationTitle.value}
            </a>
          ))}
        </nav> */}
